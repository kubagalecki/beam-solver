...
{
    // Define beam sizes
    const std::vector<std::pair<size_t, size_t>> size_span = {
            {100, 100},
            {20, 500},
            {500, 20}
    };
        ...
        for (const auto& load : case_span)
            for (const auto& number_of_threads : thread_span)
                benchmarks.push_benchmark(
                    [mx_ = size.first, my_ = size.second, l = load](){mx = mx_; my = my_; allocate_globals(); l();},
                    deallocate_globals,
                    [d = d_global](double* a1, const size_t a2){solve(a1, a2); return a1;},
                    validate, std::reference_wrapper(d_global), number_of_threads
                    );
