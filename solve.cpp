...
Iterator accumulate_consecutive(const Iterator& first_, const Iterator& last_, const Equality& eq_op, const Accumulate& ac_op)
{
    // Return if empty
    if (first_ == last_)
        return last_;
...
Block1D<T, SIZE> operator*(const Block2D<T, SIZE>& mat, const Block1D<T, SIZE>& vec)
{
    ...
    std::transform(mat.m_data.cbegin(), mat.m_data.cend(), prod_vec.m_data.begin(),
        [&vec](const std::array<T, SIZE>& mat_row) {
            return std::inner_product(mat_row.cbegin(), mat_row.cend(), vec.m_data.cbegin(), 0.);
        });
...
{
    Block1D<T, SIZE> sum{};
    std::transform(a.m_data.cbegin(), a.m_data.cend(), b.m_data.cbegin(),
        sum.m_data.begin(), [](const T& v1, const T& v2) { return v1 + v2; });
    return sum;
}
...
// Block sparse matrix type. T denotes the block type. For simplicity, matlab storage format is used, but with row-major order.
template <class T>
...
BlockSparseMatrix<T> BlockSparseMatrix<T>::assembleBSMatrix(data_t&& data,
        const index_t& r, const index_t& c, const size_t& n_parts)
{
...
template<class T>
BlockSparseMatrix<T>::BlockSparseMatrix(BlockSparseMatrix::data_t&& d, const BlockSparseMatrix::index_t& r,
        const BlockSparseMatrix::index_t &c, const size_t& n_parts) :
        m_data(std::forward<BlockSparseMatrix::data_t>(d)), m_rows(r), m_cols(c)
{
    ...
    for (size_t i = 1; i < n_parts; ++i)
    {
        ...
        m_partition.push_back(
                std::lower_bound(m_data.cbegin(), m_data.cend(), s_val,
                        [](const entry_t& b, const index_t& ind){return b.first.first < ind;}));
    }
...
template <class T2D, class T1D>
void mat_vec_prod(const BlockSparseMatrix<T2D>& mat, const std::vector<T1D>& vec, std::vector<T1D>& ret_vec,
        const std::shared_ptr<ThreadPool>& thread_pool)
{
    ...
    const auto mult = [&](auto iter_begin, const auto& iter_end)
    {
        ...
        {
            ret_vec[iter_begin->first.first] += ((iter_begin->second) * vec[iter_begin->first.second]);
            ++iter_begin;
        }
    };
...
BlockSparseMatrix< Block2D<double, 2> > assembleStiffnessMatrix(const size_t n_part = 1)
{
        ...
        for (size_t col_ind = 0; col_ind < my; col_ind++)
        {
            for (size_t i = 0; i < 4; i++)
            {
                ...
                for (size_t j = 0; j < 4; j++)
                {
                    ...
                    if (dof_i != dof_j && ((fix[2 * dof_i] && fix[2 * dof_i + 1]) || (fix[2 * dof_j] && fix[2 * dof_j + 1])))
                        continue;
    ...
    return BlockSparseMatrix< Block2D<double, 2> >::assembleBSMatrix(std::move(entries), (mx + 1) * (my + 1), (mx + 1) * (my + 1), n_part);
}
    ...
    const double tol = 1e-6)
{
    const auto minus = [](const std::vector<T1D>& a, const std::vector<T1D>& b)
    {
        ...
        std::transform(a.cbegin(), a.cend(), b.cbegin(), sum.begin(), [](const T1D& b1, const T1D& b2) { return b1 - b2; });
        return sum;
    };
    ...
    const auto increment_by_scaled_v = [](std::vector<T1D>& a, const std::vector<T1D>& b, const double& s)
    {
        std::transform(a.cbegin(), a.cend(), b.cbegin(), a.begin(), [&s](const T1D& b_a, const T1D& b_b) { return b_b * s + b_a; });
    };

    const auto residual = [&minus](const BlockSparseMatrix<T2D>& A, const std::vector<T1D>& b,
    ...
    for (iter = 0; iter < maxit; ++iter)
    {
        ...
        std::transform(p.cbegin(), p.cend(), r.cbegin(), p.begin(), [s = rxr / rxr_p](auto p, auto r){
            return p * s + r;
        });
        rxr_p = rxr;
    }
...
void solve(double* wynik, const size_t nt)
{
    ...
    const auto d = cg(K_global, F_block, decltype(F_block){F_block.size()}, std::make_shared<ThreadPool>(nt - 1));
